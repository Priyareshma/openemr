...
#IfMissingColumn form_vitals sample_note
ALTER TABLE `form_vitals` ADD `sample_note` VARCHAR(255) DEFAULT NULL;
#EndIf

#IfNotTable form_vital_options
CREATE TABLE `form_vital_options` (
`id` bigint(20) NOT NULL AUTO_INCREMENT,
`form_id` bigint(20) NOT NULL COMMENT 'FK to vital_forms.id',
`vitals_column` varchar(64) NOT NULL COMMENT 'Column name from form_vitals',
`vital_option_list_id` varchar(100) DEFAULT NULL COMMENT 'FK to list_options.list_id for vital_option',
`vital_option_option_id` varchar(100) DEFAULT NULL COMMENT 'FK to list_options.option_id for vital_option',
`vital_option_codes` varchar(255) DEFAULT NULL COMMENT 'Archived original codes value from list_options vital_option',
`vital_option_title` varchar(255) DEFAULT NULL COMMENT 'Archived original title value from list_options vital_option',
PRIMARY KEY (`id`),
KEY `fk_form_id` (`form_id`),
KEY `fk_list_options_id` (`vital_option_list_id`, `vital_option_option_id`)
) ENGINE=InnoDB COMMENT='Detailed information of each vital_forms vital_option column';
#EndIf
