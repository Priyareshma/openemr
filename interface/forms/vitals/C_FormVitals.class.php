...
class C_FormVitals extends Controller
{
    ...
    public function __construct($template_mod = "general")
    {
        ...
        $this->vitaloptionList = $this->get_vital_option_list_options();
...
//        $this->assign("gbl_vitals_options", $GLOBALS['gbl_vitals_options']);
    ...
    public function default_action()
    {
        ...
        $this->assign('vital_options', $this->vitaloptionList);
    ...
    private function get_vital_option_list_options()
    {
        $listService = new ListService();
        $options = $listService->getOptionsByListName(FormVitals::LIST_OPTION_VITAL_OPTION);
        $orderedList = [];
        foreach ($options as $option) {
            $item = [
                'title' => $option['title']
    ...
    private function get_vital_option_list_as_hash()
    {
        $hashList = [];
        foreach ($this->vitaloptionList as $option) {
            $hashList[$option['id']] = $option;
        }
        return $hashList;
    }
    ...
    public function populate_object(&$obj)
    {
        ...
        $detailsToUpdateVitalOption = array();
        ...
        if (isset($_POST['vital_option'])) {
            $vitaloptionList = $this->get_vital_option_list_as_hash();
            // grab our default list options
            foreach ($_POST['vital_option'] as $column => $value) {
                 $details = $this->vitals->get_details_for_column($column) ?? new FormVitalDetails();
                ...
                if (empty($value)) {
                    $details->clear_interpretation();
                } else if (isset($vitaloptionList[$value])) {
                    $vital_option = $vitaloptionList[$value];
                    $details->set_interpretation_list_id(FormVitals::LIST_OPTION_VITALS_INTERPRETATION);
                    $details->set_interpretation_option_id($value);
                    $details->set_interpretation_codes($value); // for now the option_id is the code
                    $details->set_interpretation_title($vital_option['title']);
                }
                else {
                    (new SystemLogger())->error(
                        "Passed in interpretation does not exist in list options, clearing interpretation id",
