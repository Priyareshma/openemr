...
use OpenEMR\Common\Forms\FormVitalOptions;
...
class VitalsService extends BaseService
{
    ...
    public function search($search, $isAndCondition = true)
    {
        $sqlSelect = "
                    ...
                    ,vitals.primary_pain_intensity
                    ,vitals.primary_pain_location
                    ,vitals.primary_pain_laterality
                    ,vitals.primary_pain_radiation
                    ,vitals.sample_note
        ...
        while ($row = sqlFetchArray($statementResults)) {
            ...
            // $options = isset($record['options']) ? array_pop($record['options']) : null;
            ...
            // if (!empty($options)) {
            //     $recordsById[$row['form_id']]["options"][$options['vitals_column']] = $options;
            // }
        }
    ...
    protected function createResultRecordFromDatabaseResult($row)
    {
        ...
        // $optioncolumns = ['options_id', 'vital_option_codes', 'vital_option_title', 'vitals_column'
        // , 'vital_option_list_id', 'vital_option_option_id'];
        ...
        //     $options[$column] = $record[$column] ?? null;
        //     unset($record[$column]);
        // }
        // if (isset($options['options_id'])) {
        //     // for anything ORM or active record (such as vitals form) we want the details to use the simplified id
        //     // instead of the longer named value
        //     $options['id'] = $options['options_id'];
        //     unset($options['options_id']);
        //     $record['options'] = [$options];

        // }
        // $record =$record+$details+$options;
        return $record;
    ...
    public function saveVitalsArray(array $vitalsData)
    {
        ...
        // $options = $vitalsData['options'] ?? [];
        // unset($vitalsData['options']);
        ...
        //     $option['form_id'] = $vitalsData['id'];
        //     $updatedOptions[$column] = $this->saveVitalOptions($option);
        // }

        $vitalsData['details'] = $updatedDetails;
        // $vitalsData['options'] = $updatedOptions;
    ...
    // private function saveVitalOptions(array $vitalOptions)
    // {
    //     $id = $vitalOptions['id'] ?? null;
    //     $sqlOperation = "UPDATE ";
    //     if (empty($id)) {
    //         $sqlOperation = "INSERT INTO ";
    //     }

    //     unset($vitalOptions['id']);
    //     $keys = array_keys($vitalOptions);
    //     $values = array_values($vitalOptions);
    //     $fields = array_map(function ($val) {
    //         return '`' . $val . '` = ?';
    //     }, $keys);
    //     $sqlSet = implode(",", $fields);

    //     $sql = $sqlOperation . FormVitalOptions::TABLE_NAME . " SET " . $sqlSet;
    //     if (!empty($id)) {
    //         $sql .= " WHERE `id` = ? ";
    //         $values[] = $id;
    //     }
